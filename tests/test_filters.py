...
def test_properties():
    assert items_equal([p.name for p in pdir(inst).properties.pattrs], [
...
def test_methods():
    if sys.version[0] == '2':
        assert items_equal([p.name for p in pdir(inst).methods.pattrs], [
...
def test_public():
    assert items_equal([p.name for p in pdir(inst).public.pattrs], [
...
def test_own():
    assert items_equal([p.name for p in pdir(inst).own.pattrs], [
...
def test_filters_with_search():
    def test_chained_filters():
        assert items_equal([
            p.name for p in
            pdir(inst).public.own.properties.search('derived_in').pattrs
        ], [
            'derived_instance_variable',
        ])
